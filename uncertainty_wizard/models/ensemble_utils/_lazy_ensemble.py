...
def _store_to_temp_file(x: np.ndarray) -> str:
...
class LazyEnsemble(_UwizModel):
    ...
    ** Stability of Lazy Ensembles **
    ...
    def _num_processes_or_default(self, num_processes: Optional[int]) -> int:
    ...
    def fit(
        ...
        val_paths: Optional[Tuple[str, str]] = None
        if pickle_arrays:
            # Temporarily save arrays to file system
            x_path = _store_to_temp_file(x)
            y_path = _store_to_temp_file(y)
            if validation_data is not None:
                val_x_path: str = _store_to_temp_file(validation_data[0])
                val_y_path: str = _store_to_temp_file(validation_data[1])
    ...
    def quantify_predictions(
            ...
            as_confidence: Optional[bool] = None,
